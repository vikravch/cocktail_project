...
import {getCocktail} from "../use_case/getCocktail.ts";
import type {CocktailShort} from "../types/CocktailShort.ts";

const DetailPane: React.FC = () =>
{
  const [cocktailData, setCocktailData] = useState<CocktailShort>();

  useEffect(()=>{
    // 11007
    getCocktail("17225").then(result => {
      console.log(result);
        ...
        <div className="detail-content-card">
          ...
          <div className="detail-header-row">
            <div>
              ...
              <h1 className="detail-title">{cocktailData.name}</h1>
              <div className="glass-type">
                <span className="material-symbols-outlined">liquor</span>
                <span>{cocktailData.description}</span>
              </div>
            </div>
